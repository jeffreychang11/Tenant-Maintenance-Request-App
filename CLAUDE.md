...
**Monthly maintenance message allowance meter moved from Settings to
Billing** (`app/(landlord)/billing/page.tsx`, removed from
`app/settings/page.tsx`) — discussed with the user first: it's plan-usage
data tied to tier limits, so it now sits right after the existing
"You currently have N units" line and before the plan cards, next to the
actions that actually resolve it (`MessageCapUpgradeButton`, shown when
`usage.blocked`). `getMessageUsage(supabase)` is fetched alongside the
existing `sub`/`unitCount` queries in the same `Promise.all`. Settings
no longer imports `getMessageUsage`/`MessageUsageBar`/
`MessageCapUpgradeButton` at all — the rooster-run animation (see above)
now fires on Billing page visits instead of Settings visits, unchanged
otherwise.

### Nav logo icon + dark mode removed (later session)

**New square logo icon added to the nav wordmark.** `public/logo-icon.png`
(128×128) — navy background (`#0F2042`), a white rooster-on-a-house-roof
line-art mark, developed through several rounds of user-provided
reference images and iteration on framing/scale (documented only in
chat history, not in this file, since the artwork itself isn't
generated by app code). `components/layout/Logo.tsx` now renders it via
`next/image` at 18×18 (sized to match the wordmark's text height, not
the icon's own native proportions) immediately left of the "Simple"/
"Roost" text, shared by both `LandlordNavBar.tsx` and `TenantNavBar.tsx`
since they both render `<Logo>`. The "Roost" half of the wordmark text
is hardcoded to the same navy (`text-[#0F2042]`) in both light and dark
mode, rather than following the existing black/white theme toggle the
"Simple" half still uses — a deliberate ask to tie the text to the icon's
fixed brand color. Only ever tested/shipped as a **favicon/app-icon
candidate**, not yet wired into `lib/pwaIcon.tsx` (the actual PWA
icon/favicon generation, still the old code-generated plain house glyph)
— revisit that file if/when this artwork is confirmed as the final brand
mark, since a 16px browser-tab favicon specifically needs a much simpler
single-shape fallback (a full rooster+house scene doesn't survive
16×16 — verified by rasterizing it at true 16px with no smoothing tricks
during design; a bold two-shape house-only mark did survive at that
size and is the intended favicon-only fallback once this ships for
real).

**Dark mode disabled app-wide** (`app/globals.css`). Was previously
automatic based on the OS/browser's `prefers-color-scheme` — there was
never a manual in-app toggle, just ~44 files' worth of `dark:` Tailwind
utilities plus a raw `@media (prefers-color-scheme: dark)` block
overriding `--background`/`--foreground`. Rather than touching every one
of those ~44 files, added `@custom-variant dark (&:where(.dark, .dark
*));` right after the Tailwind import — this switches Tailwind's `dark:`
variant from the default media-query strategy to a class-based one, and
since nothing in this app ever adds a `.dark` class anywhere, every
existing `dark:`-prefixed utility across the whole codebase is now
permanently inert rather than needing to be stripped out individually.
The plain-CSS `@media (prefers-color-scheme: dark)` block was deleted
outright (the only place dark colors could apply outside of Tailwind's
variant system). Verified live: with the browser's own color scheme
forced to dark (`window.matchMedia('(prefers-color-scheme: dark)').matches
=== true`), the page still rendered solid white body background /
